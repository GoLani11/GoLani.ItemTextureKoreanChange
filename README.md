...
원본 해상도·UV 배치·Diffuse 알파를 유지합니다. 문자 영역만 합성하는 방식과 생성한 전체 D를 채택하는 방식을 구분합니다.
...
프로젝트 전용 `$localize-spt-textures` 스킬로 품목 한글화를 요청할 수 있습니다.
기본은 원본 D 전체를 참고해 판독 가능한 문구를 한글화한 시안을 보여주는 단계까지입니다.
선택한 시안으로 크기 보정·N/G 처리·테스트 패키징을 후속 요청에 맞춰 진행합니다.
[스킬 지침](.agents/skills/localize-spt-textures/SKILL.md)에 단계별 동작이 있습니다.
...
검사는 크기·보존 채널·공유 맵과 선택한 제작 방식의 조건을 확인합니다.
`masked`는 마스크 밖 픽셀·UV 경계 변경을 차단하고, 전체 D는 해당 변화량을 보고합니다. N/G는 계속 보존 검사를 적용합니다.
비교 이미지는 `reports/`에 저장합니다.
