...
DEFAULT_INPUT = PROJECT_ROOT / "workspace" / "catalog" / "images"
DEFAULT_MANIFEST = PROJECT_ROOT / "workspace" / "catalog" / "manifest.json"
DEFAULT_OUTPUT = PROJECT_ROOT / "workspace" / "ocr"
...
def _source_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        dest="inputs",
        metavar="PATH",
        help="이미지 파일/폴더. 여러 번 지정 가능 (기본: workspace/catalog/images)",
    )
    parser.add_argument(
        "--manifest",
        metavar="PATH",
        help="기존 workspace/catalog/manifest.json 또는 범용 JSON/JSONL manifest",
    )
    parser.add_argument(
        "--no-manifest",
        action="store_true",
        help="기본 workspace/catalog/manifest.json을 사용하지 않고 입력 폴더를 직접 탐색",
    )
    parser.add_argument(
...
def _output_option(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        metavar="PATH",
        help="캐시/실행 결과 루트 (기본: workspace/ocr)",
    )
