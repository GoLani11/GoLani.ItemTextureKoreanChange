# 아이템 텍스처 한글화 자동화 설계 (폐기된 legacy 문서)

> 이 문서는 과거 `tools/auto.py` 흐름을 설명하는 기록이에요. 현재 작업과 배포에는 사용하지
> 않아요. 현행 절차는 [architecture-v2.md](architecture-v2.md)와 저장소 전용 스킬을 따르세요.
> 특히 자동 리사이즈, 전체 GPT 재생성, 파일명 기반 D/N/G 추정과 루트 `bundles/` 배포는
> 금지돼요.
