...
5. 이미지 편집: AI가 해시 고정 old-text 영역의 배경만 복구해 고정한 뒤, 영역별 원본 스타일
   crop과 확정 문자열을 기준으로 맞춤 한글 레터링 후보를 만든다. OCR과 시각 유사도로
   선택한 레터링 패치만 원본 좌표에 합성하며 고정 한글 폰트 조판은 최종물로 사용하지 않는다.
