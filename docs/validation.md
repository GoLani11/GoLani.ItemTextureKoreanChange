...
기존 작업과 `diffuse_mode: masked`는 위의 모든 픽셀 보존 조건을 적용합니다.
`adopt-draft`로 기록한 `generated-full`의 D는 채택 기록과 후보 해시, 원본 크기·RGBA·알파·공유 맵을 검사합니다.
RGB·마스크 밖·UV 경계의 변경 픽셀 수는 진단값이며 그 수가 0이어야 한다는 조건을 적용하지 않습니다.
이때 `diffuse_preservation`은 `unverified-nontext`이고 비문자 픽셀 보존을 검증했다고 표시하지 않습니다.
N/G에는 기존 마스크·UV·채널 검사를 적용하고, 수정된 맵은 현재 D와 글자 윤곽의 파생 기록도 확인합니다.
알 수 없는 정책 값, 채택 기록이 없는 전체 D, 변경된 기준 파일은 실패합니다.
...
**무손실 보존의 범위:** `masked`의 PNG 후보와 두 방식의 N/G는 마스크 밖 픽셀이 원본과 같아야 합니다.
전체 생성 D는 비문자 RGB 보존을 보증하지 않습니다. 압축 뒤에는 4×4 블록 단위로
...
검사와 패키지 기록에는 `diffuse_mode`와 `diffuse_preservation`을 포함하고 `TEST-ONLY.txt`에도 제작 방식을 표시합니다.
파일 검사 합격은 해당 제작 방식의 조건을 통과했다는 뜻이며 시각·게임 확인을 뜻하지 않습니다.
