...
3. 플러그인이 공식 `review_record.py`의 analysis 게이트, profile의 bundle/Texture2D identity,
   원본·review·5종 마스크 SHA를 다시 검사해요. 목록에는 `형식 준비됨·공식 게이트·SHA 검사
   대기`, `analysis 갱신 필요`, `마스크 준비 필요`를 구분해 표시해요. 목록 새로고침은 빠른
   형식 검사이고, 불러오기에서 공식 게이트와 모든 파일 SHA를 다시 확인해요.
4. 준비된 품목은 불변 원본 Diffuse를 열고 현재 라벨 면의 `editable` 픽셀만 Krita 선택으로
...
8. 원본 배율에서 결과를 보고 `이 결과 선택` 또는 `결과 제외`를 눌러요. 선택은
