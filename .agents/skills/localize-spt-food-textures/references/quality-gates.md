...
- 원본 스타일 crop, 깨끗한 배경, AI 후보 시트, 선택 레터링·마스크와 모델 서명을 영역별로
  해시 고정해야 한다.
- 각 영역은 비교 가능한 AI 레터링 후보를 최소 2개 기록해야 한다.
- `compositor.mode`는 `ai-reference-lettering`, `fixed_font_used`는 `false`,
  `background_locked`는 `true`여야 한다.
- 고정 한글 폰트 또는 일반 글꼴의 단순 합성을 최종 레터링으로 사용하면 안 된다.
...
- 글자 실루엣·획 성격·장평·자간·기울기·외곽선·그림자·입체감·마모와 인쇄 질감이 원본
  스타일 참조와 맞아야 한다.
- 기존 원문 로고의 글자뿐 아니라 외곽선·그림자·배경 실루엣도 남아 있으면 안 된다.
...
- AI 레터링 manifest의 문자열과 횟수가 명세와 같아야 한다.
- 후보 OCR과 Codex 시각 비교를 둘 다 수행해야 한다.
- 금지된 라틴·키릴 잔상, 한국어 누락·오자·중복이 없어야 한다.
- 영역별 글자 형태·크기·방향·간격·효과·표면 통합 판정이 모두 통과해야 한다.
