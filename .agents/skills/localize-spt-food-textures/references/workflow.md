...
- 글자 실루엣, 획 성격, 장평, 기울기, 자간, 외곽선, 그림자와 입체 효과
...
[ai-lettering.md](ai-lettering.md)를 따라 배경 복구와 한글 레터링을 분리한다. 먼저 AI로
`old_text` 안의 원문만 제거하고 승인한 깨끗한 배경을 해시 고정한다. 다음에는 영역별 원본
확대 crop, 고정 배경, 확정 한국어, bbox와 방향을 AI에 제공해 원본 스타일의 맞춤 한글
레터링을 생성한다. 고정 한글 폰트를 선택하거나 일반 글꼴을 얹지 않는다.

영역별 최소 2개 후보를 한 번에 생성하고 OCR 철자 검사 뒤 원본과 같은 배율로 실루엣,
크기, 장평, 방향, 간격, 외곽선, 그림자와 표면 질감을 비교한다. 선택한 레터링의 승인 마스크
안 픽셀만 고정 배경에 합성한다. 다음 순서와 모든 입력 해시로 최종 후보를 재현할 수 있어야
한다.

```text
원본 + old_text 안의 고정 배경 patch + new_text 안의 선택 AI 레터링 patch
```

원본 스타일 crop, 고정 배경, 후보 시트, 선택 패치·마스크, 모델 서명, 확정 문자열과 합성
좌표를 기록한다. 원본 전체 재생성, 전체 Texture2D 리사이즈, 후보별 배경 재생성을 금지한다.
현재 `localize.py compose`의 고정 폰트 조판 결과는 이 단계의 최종 후보로 인정하지 않는다.
...
2. Codex 시각 비교: 원본과 후보를 같은 배율로 비교하고 글자·그림 방향, 색, 질감, 흐림,
   seam·절취선, 로고와 비문자 구조에 더해 글자 실루엣·크기·장평·간격·외곽선·그림자,
   표면 통합과 원문 로고 실루엣 잔존을 확인한다.
...
- 기존 외국어 relief나 광택이 있을 때만 각 보조맵의 명시적 재질 전용 `old_text` 영역을
  ...
  자동 재사용하지 않는다. `material_masks`에는 맵별 경로·SHA-256과 `inpaint` 또는 `patch`
  방식을 기록한다. `patch`는 원본과 같은 모드·크기의 프로젝트 내부 이미지와 SHA-256을
  함께 고정하며, 마스크 안 픽셀에만 적용한다.
