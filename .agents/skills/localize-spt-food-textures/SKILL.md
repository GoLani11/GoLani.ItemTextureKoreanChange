...
- 작업을 `빠른 초안`과 `최종 검증`으로 분리하라. 초안에서는 생성 패널을 원본 배율로 보고
  글자 인상·자연스러운 자간·라벨 균형·비문자 흔들림만 선별하라. 패널 OCR, 공식 합성,
  후보 게이트와 downstream 검사는 실행하지 마라. 초안은 `검증 전 미리보기`로만 표시하라.
- 품목의 연결된 라벨 면마다 기본 생성 예산을 2회로 제한하라. 첫 결과에서 시각 실패 원인
  하나만 고쳐 두 번째 결과를 만들고, 그래도 시각적으로 부적합하면 자동 재시도를 중단해
  `block`으로 보고하라. 사용자가 명시적으로 추가 시도를 요청한 경우에만 예산을 늘려라.
- 패널 OCR은 시각적으로 채택한 생성 패널 SHA에만 실행하고, 통과한 패널만 공식 합성하라.
  후보 OCR은 합성·픽셀·시각 사전 검사를 통과한 후보 SHA에만 실행하라. OCR 실패 시 남은
  생성 예산 안에서 해당 원인만 한 번 보정하고 새 SHA에 다시 실행하라.
- 후보 게이트가 통과하기 전에는 Material, D/N/G, 밉, 압축, 번들, 실제 렌더 검사를 시작하지
  마라. 승인 후보 SHA가 고정된 뒤 downstream 단계를 각각 한 번씩 실행하라.
- 입력 파일·설정·도구 서명과 보고서 SHA가 모두 같으면 기존 통과 증거를 재사용하라. 입력이
  변하지 않은 OCR·합성·재질·밉·번들 명령을 다시 실행하지 마라.
- 명령의 상세 JSON과 로그는 파일에 저장하고 대화에는 상태, 실패 이유, 핵심 수치와 경로만
  출력하라. 디버깅에 필요한 필드만 선택해 읽고 사용자가 요청하지 않은 전체 보고서를 대화에
  복사하지 마라.
...
7. 이미지 생성 도구에는 원본 전체와 한 번에 이해할 수 있는 연결된 라벨 면을 제공하라. 같은
   ...
   의무 생성하지 마라. 대상 문구가 정방향이 아니면 고정 90°가 아니라 기록한 원문 회전의
   역각으로 연결된 라벨 면의 임시 작업 패널을 정방향화해 생성과 OCR을 실행하라. 승인한 글자
   패치와 마스크만 작업 변환의 정확한 역변환으로 원본 좌표·방향에 복원하라. 원본이나 최종
   Texture2D 전체를 회전·재표본화하지 마라. 첫 생성 패널은 원본 배율 시각 비교부터 실행하라.
   시각적으로 채택한 생성 패널에만 패널 OCR을 실행해 공식 합성하고, 합성 후보가 픽셀·시각
   사전 검사를 통과한 뒤 후보 OCR을 실행하라. 생성 예산 안에서 실패 원인 하나씩만 보정하라.
   패널 OCR 전 결과는 `검증 전 미리보기`일 뿐 최종 후보가 아님을 명시하라.
...
9. 각 영역의 한글은 원문과 다음 특성을 거의 같게 만들어라.
   ...
   수치상 bbox를 맞추려고 음절 사이에 과도한 공백을 만들거나 글자를 인위적으로 늘리지 마라.
   수치 검사 전에 원본 배율 비교로 자연스러운 단어 형태와 라벨 균형을 먼저 확인하라.
...
- 대화에는 전체 JSON 대신 단계별 한 줄 요약과 핵심 실패 수치만 쓰고, 상세 증거는 경로로
  연결하라. 재시도 횟수와 재사용한 SHA 고정 결과를 함께 요약하라.
