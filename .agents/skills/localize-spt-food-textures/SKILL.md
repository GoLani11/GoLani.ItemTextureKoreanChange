...
원본은 Codex가 원본 해상도에서 먼저 시각 판독하고, 모호한 영역에만 원본 OCR을 보조로
사용하라. 한글은 원문의 글꼴 인상·스타일·크기·실루엣·배치·방향을 거의 같게 유지하고,
결과 OCR과 최종 시각 비교가 모두 통과하지 않으면 다음 단계로 진행하지 마라.
...
2. 원본 mip 0을 `view_image`의 원본 상세도로 한 번 보고 모든 대상 글자를 Codex가 먼저
   판독하라. 문자열·bbox·회전·읽는 방향·면·그림 방향과 글꼴 인상, 획, 비례, 기준선,
   정렬, 자간, 외곽선, 그림자, 마모를 `source_visual`에 기록하라.
3. 명확히 읽히는 영역에는 원본 OCR을 실행하지 마라. 확대해도 문자가 모호하거나 누락 가능성이
   있는 영역만 `needs_ocr_fallback: true`로 표시하고 그 영역에 한해 OCR과 교차검증을 실행하라.
   OCR과 시각 판독이 충돌하면 생성하지 마라.
...
7. 이미지 생성 도구에는 원본 전체와 한 번에 이해할 수 있는 연결된 라벨 면을 제공하라. 같은
   ...
   의무 생성하지 마라. 첫 결과를 검사하고 실패 원인 하나만 고쳐 한 번 재시도할 수 있다.
...
11. 결과 OCR로 확정 한국어의 철자·횟수와 금지 외국어 잔상을 검사하라. 이어서 Codex가
...
- Codex 시각 판독에 읽지 못한 문구가 남았는데 원본 OCR·교차검증을 하지 않았다.
